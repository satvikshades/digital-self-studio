...
export default function AvatarViewer({ originalImage, generatedImage, onRetry }: AvatarViewerProps) {
  ...
  return (
    <div className="animate-fade-in">
      ...
      <div className="mt-6 flex flex-col sm:flex-row items-center justify-center gap-3">
        <Button size="lg" onClick={handleDownload} className="w-full sm:w-auto rounded-full">
          <Download className="w-4 h-4 mr-2" />
          Download
        </Button>

        {typeof navigator !== "undefined" && navigator.share && (
          <Button variant="outline" size="lg" onClick={handleShare} className="w-full sm:w-auto rounded-full">
            <Share2 className="w-4 h-4 mr-2" />
            Share
          </Button>
        )}

        <Button variant="outline" size="lg" onClick={onRetry} className="w-full sm:w-auto rounded-full">
